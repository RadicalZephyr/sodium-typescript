...
export class Source {
    ...
    deregister(_target : Vertex) : void {
        if (this.registered) {
...
export class Vertex {
    ...
    static collectCycles() : void {
        ...
        try {
            ...
            for (let i = Vertex.toBeFreedList.length-1; i >= 0; --i) {
                const vertex = Vertex.toBeFreedList.splice(i, 1)[0];
                vertex.free();
            }
        } finally {
    ...
    static markRoots() : void {
        ...
        if (verbose) {
            const stack: Vertex[] = roots.slice(0);
